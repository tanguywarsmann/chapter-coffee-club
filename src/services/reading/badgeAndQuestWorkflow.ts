...
import { clearProgressCache } from "./progressService";
...
export async function handleBadgeAndQuestWorkflow(request: any, progressId: string, clampedPage: number, nextSegment: number, book_id: string, user_id: string, question: any) {
  ...
  await Promise.all([
    queryClient.invalidateQueries({ queryKey: queryKeys.readingProgress(user_id) }),
    queryClient.invalidateQueries({ queryKey: queryKeys.bookProgress(book_id) }),
    queryClient.invalidateQueries({ queryKey: queryKeys.jokersInfo(book_id, user_id) }),
  ]);
