    ...
    const steps: Step[] = [
      ...
      },
      {
        ...
        run: async () => {
          ...
          if (error) throw error;
          if (!data || !data.length) {
            log('Pas de livre à mettre à jour → on saute cette étape'); return;
          }
      ...
      },
      {
        ...
        run: async () => {
          ...
          if (!p) {
            log('Skip nettoyage (pas de path enregistré)'); return;
          }
      ...
      <AdminGuard>
        <AdminLayout>
          <div className="max-w-3xl mx-auto p-6">
            <h1 className="text-h3 font-semibold mb-2">Audit Supabase (Admin)</h1>
            <p className="text-body-sm text-muted-foreground mb-4">
              Vérifie RLS (lecture), génération JPEG, Storage (upload/remove) et update DB (avec rollback).
            </p>
            <div className="flex items-center gap-2 mb-4">
              <button
                className="border rounded px-3 py-2 text-body-sm"
                onClick={runAudit}
            ...
            <div className="text-body-sm p-3 rounded border bg-muted/50 whitespace-pre-wrap">
              {logs.length ? logs.join('\n') : '—'}
            </div>

            {status === 'ok' && (
              <div className="mt-3 text-green-700 text-body-sm">Tout est opérationnel ✅</div>
            )}
            {status === 'fail' && (
              <div className="mt-3 text-red-700 text-body-sm">Audit en échec. Regarde les logs ci-dessus.</div>
            )}
          </div>
        </AdminLayout>
      </AdminGuard>
