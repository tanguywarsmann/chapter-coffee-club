-- A1) Backfill book_id à partir du slug (inchangé)
...
WITH d AS (
  SELECT book_id, segment, MIN(id) AS keep_id
  FROM public.reading_questions
  WHERE book_id IS NOT NULL AND segment >= 1
  GROUP BY book_id, segment
  HAVING COUNT(*) > 1
)
DELETE FROM public.reading_questions rq
USING d
WHERE rq.book_id = d.book_id
  AND rq.segment = d.segment
  AND rq.id <> d.keep_id;

-- A2) Garde-fous: unicité et perfs (inchangé)
...
-- A5) Vue publique légère (sans answer) pour lectures côté client (inchangé)
